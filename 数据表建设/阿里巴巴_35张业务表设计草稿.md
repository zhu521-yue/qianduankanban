...
7. 只有`订单状态 = 已发货`的记录进入销售及退款汇总。
8. `transaction_amount = ROUND(COALESCE(销售金额, 0), 2)`；`销售金额`是行级交易金额，不再乘销售数量。
9. `refund_amount = ROUND(COALESCE(退货金额, 0), 2)`；`退货金额`是行级退款金额，不再乘实退数量。
10. `transaction_date`取`付款日期`的北京时间日期，格式为`YYYY-MM-DD`。
11. 客户拿货频次按周期内不同交易日期数计算，即`COUNT(DISTINCT transaction_date)`。
12. 客户健康度按连续自然周生成；跨月周的月拿货频次取两个自然月拿货频次的平均值。
13. 客户健康度状态与快团团一致，但状态说明、跟进动作及API配置属于分销组独立配置，不与私域组共用内容。
...
| 空`实退数量` | 552,830 | 252,872 | 805,702，仅影响退款数量，不影响退款金额 |
| 空`实退金额` | 560,120 | 256,074 | 816,194，字段原样保留，不参与退款金额计算 |
...
| 交易金额 | `销售金额` | `transaction_amount` | `NUMERIC(18,2)` | `ROUND(COALESCE(销售金额,0), 2)`，不乘数量 |
| 实退数量 | `实退数量` | `actual_refund_quantity` | `NUMERIC(18,4)` | 原始空值保留；计算时按0 |
| 退款金额 | `退货金额` | `refund_amount` | `NUMERIC(18,2)` | `ROUND(COALESCE(退货金额,0), 2)`，不乘数量 |

### 3.2 订单状态口径

- `raw_data`无条件保留`已取消`和`已发货`原始记录。
- 只有`order_status = '已发货'`进入销售、退款、客户、商品和健康度下游汇总。
- `已取消`只保留在`raw_data`，不计入销售额、退款额、拿货频次和商品数量。

### 3.3 金额与空值口径

```text
product_quantity   = COALESCE(销售数量, 0)
transaction_amount = ROUND(COALESCE(销售金额, 0), 2)

actual_refund_quantity = COALESCE(实退数量, 0)
refund_amount          = ROUND(COALESCE(退货金额, 0), 2)
```

- 原始字段空值在`raw_data`原始列中保持空值，不能回写成0。
- 只有标准分析字段在派生时使用0。
- 销售金额与退款金额分别统计；销售表不直接扣减退款金额。
- 销售额不得改用`已付金额`、`应付金额`或`实发金额`；退款额不得改用`实退金额`。
...
8. `transaction_amount`必须逐行等于`ROUND(COALESCE(销售金额,0), 2)`，不得乘销售数量。
9. 空`退货金额`派生的`refund_amount`必须为0。
10. `refund_amount`必须逐行等于`ROUND(COALESCE(退货金额,0), 2)`，不得乘实退数量。
...
`已取消`和`已发货`都保留在`raw_data`；只有`已发货`参与销售、退款、商品、客户、拿货频次和健康度。当前样本有479条`已取消`。

### 2. 销售金额字段的单位含义
已确认`销售金额`本身就是行级交易金额，逐行保留2位小数后直接汇总，不再乘`销售数量`。

### 3. 退款金额字段的单位含义
已确认直接使用`退货金额`，逐行保留2位小数后汇总，不再乘`实退数量`或使用`实退金额`。
