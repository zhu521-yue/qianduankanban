...
3. 聚水潭下游分析只使用以下9个核心原始字段：`订单状态`、`分销商`、`店铺`、`付款日期`、`商品编码`、`销售数量`、`销售金额`、`实退数量`、`退货金额`。
4. `raw_data`保留`已取消`和`已发货`；只有`已发货`进入销售、退款、商品、客户及健康度下游汇总。
5. 商品编码取`商品编码`，商品数量取`COALESCE(销售数量, 0)`；日期筛选后1,567条空销售数量按0处理。
6. 交易金额严格按`ROUND(COALESCE(销售金额, 0), 2)`逐行计算；`销售金额`是行级交易金额，不乘销售数量，也不使用`已付金额`、`应付金额`或`实发金额`替代。
...
9. 退款金额严格按`ROUND(COALESCE(退货金额, 0), 2)`逐行计算，不乘实退数量；退款周期按年份纠正后的`transaction_date`归属。
...
| 销售金额为空或非法 | 0 | 当前可解析 |
| 实退数量为空 | 628,526 | 仅影响退款数量，不影响退款金额 |
| 实退金额为空 | 636,302 | 字段原样保留，不参与退款金额计算 |
| 实退数量非空但实退金额为空 | 7,776 | 不影响退款金额，退款额直接取`退货金额` |
| 实退金额为负数 | 54 | 字段原样保留，不参与退款金额计算 |
| 付款日期为空或无法解析 | 0 | 当前无此问题 |

按当前公式直接扫描得到的参考值：

- `销售数量`合计为864,357；1,567条空销售数量按0处理，因此合计不变。
- 旧乘法口径曾得到交易金额613,793,309.18和退款金额3,936,529.34，这两个结果已经废止，不能再用于看板。
- 新口径必须分别对`销售金额`和`退货金额`逐行保留2位小数后汇总，并在历史数据回刷时重新生成对账基准。
...
| `transaction_amount` | `ROUND(COALESCE(销售金额,0), 2)` | `NUMERIC(18,2)` | 销售金额为行级交易金额，不乘数量 |
...
| `refund_amount` | `ROUND(COALESCE(退货金额,0), 2)` | `NUMERIC(18,2)` | 不乘数量，按年份纠正后的`transaction_date`归属退款周期 |
...
| 38 | `成本价` | 否 | 原始成本留存 |
| 39 | `成本价来源` | 否 | 原始成本属性 |
| 40 | `销售数量` | 是 | 标准商品数量，不参与交易金额计算 |
| 41 | `实发数量` | 否 | 原始履约数量 |
| 42 | `实发金额` | 否 | 原始履约金额 |
| 43 | `销售金额` | 是 | 行级交易金额，直接汇总 |
| 44 | `销售成本` | 否 | 原始成本留存 |
| 45 | `实发成本` | 否 | 原始成本留存 |
| 46 | `销售毛利` | 否 | 原始毛利留存 |
| 47 | `销售毛利率` | 否 | 原始毛利率留存 |
...
| 51 | `基本金额` | 否 | 原始金额留存 |
| 52 | `退货数量` | 否 | 原始退货信息 |
| 53 | `实退数量` | 是 | 退款数量及退款记录识别 |
| 54 | `退货金额` | 是 | 行级退款金额，直接汇总 |
| 55 | `退货成本` | 否 | 原始退货成本 |
| 56 | `实退成本` | 否 | 原始退货成本 |
| 57 | `实退金额` | 否 | 原始字段留存，不参与退款金额计算 |
| 58 | `运费收入` | 否 | 原始运费信息 |
| 59 | `运费收入分摊` | 否 | 原始运费信息 |
| 60 | `整单运费支出` | 否 | 原始运费信息 |
| 61 | `运费支出` | 否 | 原始运费信息 |
...
- 构建思路：筛选`已发货`且付款日期、销售金额可用的记录，逐行对`销售金额`保留2位小数，再按年份纠正后的`transaction_date`汇总；销售额不直接扣减退款额。
...
- 构建思路：从`raw_data`筛选`已发货`记录，逐行对`退货金额`保留2位小数，按年份纠正后的`transaction_date`所在自然周汇总；有销售但无退款的周保留记录并写0。
- 唯一键：`period_start + period_end`。
...
7. `product_quantity`必须等于`COALESCE(销售数量,0)`；`transaction_amount`必须逐行等于`ROUND(COALESCE(销售金额,0), 2)`，不得乘销售数量。
8. `refund_amount`必须逐行等于`ROUND(COALESCE(退货金额,0), 2)`，不得乘实退数量。
...
7. 空`退货金额`按0；非零`退货金额`保留原符号，不自动取绝对值。
...
| 旧口径交易金额（已废止） | 613,793,309.18 |
| 旧口径商品交易金额（已废止） | 613,793,309.18 |
| 商品数量 | 864,357.00 |
| 旧口径退款金额（已废止） | 3,936,529.34 |
| 客户映射缺失 | 0 |
| 周期边界异常 | 0 |
| 日级直接聚合集合差异 | 0 |
...
本节金额是2026-08-12按旧乘法口径生成的历史实施记录，现已废止。切换到“销售金额/退货金额直接汇总”口径后，相关销售、退款及上级汇总表必须重新回刷并重新对账。
