# 快手小店第16张表审核草稿：`kuaishouxiaodian.quarterly_product_sales`
...
> 样例说明：本文样例直接查询正式表，完整保留`id`、`product_code`、全部业务字段、`created_at`和`updated_at`。

## 1. 表的作用

`quarterly_product_sales`用于保存每个SKU编码在每个自定义季度内的销售金额和商品数量。一条记录代表“一个自定义季度 + 一个SKU编码”的汇总结果。
...
| 6 | `quarterly_product_quantity` | `BIGINT` | 是 | `0` | 本周期商品数量之和 |
...
| `quarterly_product_quantity` | `product_quantity` | 按周期及SKU编码求和 |
...
| `id` | `period_start` | `period_end` | `product_code` | `quarterly_transaction_amount` | `quarterly_product_quantity` | `created_at` | `updated_at` |
|---:|---|---|---|---:|---:|---|---|
| 723 | 2025-02-01 | 2025-04-30 | `6941716557102` | 103,093.40 | 2,066 | 2026-08-10 11:33:08.180113+08 | 2026-08-10 11:33:08.180113+08 |
| 1,406 | 2025-05-01 | 2025-07-31 | `6941716545857` | 61,876.20 | 1,088 | 2026-08-10 11:33:08.180113+08 | 2026-08-10 11:33:08.180113+08 |
| 2,290 | 2025-08-01 | 2025-10-31 | `6941716538026` | 51,097.20 | 308 | 2026-08-10 11:33:08.180113+08 | 2026-08-10 11:33:08.180113+08 |
| 3,385 | 2025-11-01 | 2026-01-31 | `6941716573539` | 17,077.62 | 625 | 2026-08-10 11:33:08.180113+08 | 2026-08-10 11:33:08.180113+08 |
| 3,696 | 2026-02-01 | 2026-04-30 | `6941716542047` | 80,376.10 | 4,039 | 2026-08-10 11:33:08.180113+08 | 2026-08-10 11:33:08.180113+08 |
| 4,937 | 2026-05-01 | 2026-07-31 | `6941716542047` | 42,147.00 | 2,330 | 2026-08-10 11:33:08.180113+08 | 2026-08-10 11:33:08.180113+08 |
...
```sql
CREATE TABLE kuaishouxiaodian.quarterly_product_sales (
    ...
    CONSTRAINT quarterly_product_sales_period_check CHECK (
    ...
    CONSTRAINT quarterly_product_sales_product_code_check CHECK (
        BTRIM(product_code) <> ''
    ),
    CONSTRAINT quarterly_product_sales_amount_check CHECK (
        quarterly_transaction_amount > 0
    ),
    CONSTRAINT quarterly_product_sales_quantity_check CHECK (
        quarterly_product_quantity > 0
    ),
    CONSTRAINT quarterly_product_sales_business_uk UNIQUE (
...
```sql
INSERT INTO kuaishouxiaodian.quarterly_product_sales (
    period_start,
    period_end,
    product_code,
    quarterly_transaction_amount,
    quarterly_product_quantity
)
SELECT
    CASE
        WHEN EXTRACT(MONTH FROM transaction_date) BETWEEN 2 AND 4
    ...
    SUM(product_quantity)::BIGINT AS quarterly_product_quantity
...
2. 创建`kuaishouxiaodian.quarterly_product_sales`。
...
- `quarterly_transaction_amount`必须全部大于0，金额合计必须为6,247,470.14。
- `quarterly_product_quantity`必须全部大于0，数量合计必须为127,787。
...
- 不同SKU编码：3,208个。
- 销售金额合计：6,247,470.14；商品数量合计：127,787。
- 字段缺失、必填字段空值、重复业务键、非法周期、空白`product_code`、非正数金额及非正数数量：均为0条。
- 与`daily_product_sales`按相同自定义季度和`product_code`全量重算的记录、金额及数量差异：均为0。
- 表所有者：`root`。
