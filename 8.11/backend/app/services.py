...
class DashboardService:
    ...
    def dashboard(
        ...
        previous_half = previous_window(half)
        previous_month = previous_window(month)
        half_sales = self.repo.sales_amount(stores, Grain.HALF, half)
        month_sales = self.repo.sales_amount(stores, Grain.MONTH, month)
        previous_half_sales = self.repo.sales_amount(stores, Grain.HALF, previous_half)
        previous_month_sales = self.repo.sales_amount(stores, Grain.MONTH, previous_month)
        ...
        sales_trend = [
            {
                ...
                "label": window.label,
                "amount": amount_text(self.repo.sales_amount(stores, trend_grain, window)),
            }
        ...
        healthy = sum(item["count"] for item in health_items if item["status"] in {"高活跃", "活跃", "稳定"})
        health_total = sum(item["count"] for item in health_items)
        ...
        refund_period = period_window(refund_grain, as_of)
        previous_refund_period = previous_window(refund_period)
        refund_amount = self.repo.refund_amount(stores, refund_grain, refund_period)
        previous_refund_amount = self.repo.refund_amount(stores, refund_grain, previous_refund_period)
        refund_series = [
            {
                "start": window.start.isoformat(),
                "end": window.end.isoformat(),
                "amount": amount_text(self.repo.refund_amount(stores, refund_grain, window)),
            }
        ...
        return {
            ...
            "kpis": [
                {"key": "half_sales", "label": "半年销售额", "value": amount_text(half_sales), "change": ratio(half_sales, previous_half_sales), "period": half.label},
                {"key": "month_sales", "label": "本月销售额", "value": amount_text(month_sales), "change": ratio(month_sales, previous_month_sales), "period": month.label},
            ...
            "sales_trend": {"grain": trend_grain.value, "series": sales_trend},
            "customer_health": {
                "period": {"start": health_week.start.isoformat(), "end": health_week.end.isoformat(), "label": health_week.label},
                "total": health_total,
...
class SettingsService:
    ...
    def update_health_rules(self, user: UserContext, rules: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if statuses != CUSTOMER_HEALTH_STATUSES:
            raise ApiError(422, "HEALTH_RULE_STATUS_INVALID", "客户状态顺序不正确。")
        normalized_rules = []
        for rule in rules:
            ...
            normalized_rules.append({**rule, "state_instructions": state_instructions, "follow_up_action": follow_up_action})
