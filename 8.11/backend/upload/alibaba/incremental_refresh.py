...
def _prepare_scopes(conn: Connection, affected_dates: tuple[date, ...]) -> None:
    conn.execute('''
        CREATE TEMP TABLE upload_alibaba_dates (
            transaction_date date PRIMARY KEY
        ) ON COMMIT DROP
    ''')
    conn.execute(
        "INSERT INTO upload_alibaba_dates (transaction_date) SELECT DISTINCT UNNEST(%s::date[])",
        (list(affected_dates),),
    )
    conn.execute('''
        CREATE TEMP TABLE upload_alibaba_periods (
            grain text NOT NULL,
    ...
    with conn.cursor() as cursor:
        cursor.executemany(
            "INSERT INTO upload_alibaba_periods (grain, period_start, period_end) "
...
def _prepare_fact(conn: Connection) -> None:
    ...
    conn.execute("CREATE INDEX ON upload_alibaba_fact (customer_id, transaction_date)")
    conn.execute("CREATE INDEX ON upload_alibaba_fact (product_code, transaction_date)")
    conn.execute("ANALYZE upload_alibaba_fact")
    ...
    return f'''
        WITH delta AS (
            SELECT periods.period_start, periods.period_end{dimension_select},
                   ...
                   {quantity_delta}
            FROM upload_alibaba_periods periods
            JOIN upload_alibaba_fact fact
...
def _period_scope(grain: str) -> str:
    return f'''EXISTS (
        SELECT 1 FROM upload_alibaba_periods periods
...
def _refresh_health(conn: Connection) -> TableChange:
    conn.execute('''
        CREATE TEMP TABLE upload_alibaba_health_customers ON COMMIT DROP AS
        SELECT DISTINCT daily.customer_id
        FROM alibaba.customer_daily_sales daily
        JOIN upload_alibaba_dates dates USING (transaction_date)
    ''')
    conn.execute("CREATE UNIQUE INDEX ON upload_alibaba_health_customers (customer_id)")
    conn.execute('''
        CREATE TEMP TABLE upload_alibaba_health_weeks ON COMMIT DROP AS
        SELECT DISTINCT weekly.period_start, weekly.period_end
        FROM alibaba.customer_weekly_sales weekly
        JOIN upload_alibaba_periods monthly
          ON monthly.grain = 'month'
         AND weekly.period_start <= monthly.period_end
         AND weekly.period_end >= monthly.period_start
    ''')
    conn.execute(
        "CREATE UNIQUE INDEX ON upload_alibaba_health_weeks (period_start, period_end)"
    )
    conn.execute('''
        INSERT INTO upload_alibaba_health_weeks (period_start, period_end)
        SELECT period_start, period_end
        FROM upload_alibaba_periods
        WHERE grain = 'week'
    ...
    expected = '''
        WITH calendar AS (
            ...
            FROM upload_alibaba_health_customers customers
            CROSS JOIN upload_alibaba_health_weeks weeks
        ), counts AS (
            SELECT calendar.*,
    ...
    '''
    scope = '''
        target.customer_id IN (SELECT customer_id FROM upload_alibaba_health_customers)
        AND EXISTS (
...
def refresh_store_incremental(
    ...
    day_scope = "target.transaction_date IN (SELECT transaction_date FROM upload_alibaba_dates)"
    ...
    for prefix, (grain, _) in GRAINS.items():
        ...
        changes.append(_sync(conn, f"{prefix}_refunds", ("period_start", "period_end"), (
            ...
            WITH delta AS (
                ...
                FROM alibaba.{prefix}_refunds target
                JOIN upload_alibaba_periods periods
                  ON periods.grain = '{grain}'
                 AND periods.period_start = target.period_start
                 AND periods.period_end = target.period_end
    ...
    conn.execute('''
        CREATE TEMP TABLE upload_alibaba_daily_metric_dates ON COMMIT DROP AS
        SELECT DISTINCT current.transaction_date
        FROM alibaba.daily_sales current
        WHERE EXISTS (
            SELECT 1 FROM upload_alibaba_dates changed
    ...
    ''')
    daily_metric_scope = "target.transaction_date IN (SELECT transaction_date FROM upload_alibaba_daily_metric_dates)"
    changes.append(_sync(conn, "daily_sales_metrics", ("transaction_date",), (
        ...
        FROM alibaba.daily_sales current
        JOIN upload_alibaba_daily_metric_dates dates USING (transaction_date)
        LEFT JOIN alibaba.daily_sales previous
    ...
    for prefix, rate, interval, grain in (
        ...
        conn.execute(f'''
            CREATE TEMP TABLE upload_alibaba_{prefix}_metric_periods ON COMMIT DROP AS
            SELECT sales.period_start, sales.period_end
            FROM alibaba.{prefix}_sales sales
            WHERE EXISTS (
                SELECT 1 FROM upload_alibaba_periods changed
        ...
        metric_scope = f'''EXISTS (
            SELECT 1 FROM upload_alibaba_{prefix}_metric_periods periods
        ...
        changes.append(_sync(conn, f"{prefix}_sales_metrics", ("period_start", "period_end"), (
            ...
            FROM alibaba.{prefix}_sales current
            JOIN upload_alibaba_{prefix}_metric_periods periods
              USING (period_start, period_end)
            LEFT JOIN alibaba.{prefix}_sales previous
    ...
    changes.append(_sync(conn, "customer_daily_sales", ("customer_id", "transaction_date"), (
        ...
        FROM alibaba.daily_customer_sales
        WHERE transaction_date IN (SELECT transaction_date FROM upload_alibaba_dates)
    ''', day_scope))
    conn.execute('''
        CREATE TEMP TABLE upload_alibaba_customer_metric_keys ON COMMIT DROP AS
        SELECT DISTINCT current.customer_id, current.transaction_date
        FROM alibaba.customer_daily_sales current
        WHERE EXISTS (
            SELECT 1 FROM upload_alibaba_dates changed
            WHERE current.transaction_date BETWEEN changed.transaction_date
                                               AND changed.transaction_date + 29
          ...
          AND current.customer_id IN (
              SELECT DISTINCT customer_id FROM upload_alibaba_fact
              WHERE customer_id IS NOT NULL
                AND transaction_date IN (SELECT transaction_date FROM upload_alibaba_dates)
          )
    ''')
    customer_metric_scope = '''EXISTS (
        SELECT 1 FROM upload_alibaba_customer_metric_keys keys
    ...
    changes.append(_sync(conn, "customer_daily_sales_metrics", ("customer_id", "transaction_date"), (
        ...
        SELECT current.customer_id, current.transaction_date, current.transaction_amount,
               (SELECT COALESCE(SUM(item.transaction_amount),0)::numeric(18,2)
                FROM alibaba.customer_daily_sales item
               ...
               (SELECT COALESCE(SUM(item.transaction_amount),0)::numeric(18,2)
                ...
                WHERE item.customer_id = current.customer_id
                  AND item.transaction_date BETWEEN current.transaction_date - 29 AND current.transaction_date)
                    AS rolling_30_day_transaction_amount
        FROM alibaba.customer_daily_sales current
        JOIN upload_alibaba_customer_metric_keys keys USING (customer_id, transaction_date)
    ...
    for prefix, (grain, _) in GRAINS.items():
        ...
        changes.append(_sync(conn, f"customer_{prefix}_sales", (
            ...
            WITH delta AS (
                ...
                FROM alibaba.customer_{prefix}_sales target
                JOIN upload_alibaba_periods periods
                  ON periods.grain = '{grain}'
                 AND periods.period_start = target.period_start
                 AND periods.period_end = target.period_end
    ...
    for prefix, grain in (("monthly", "month"), ("quarterly", "quarter"), ("half_year", "half")):
        ...
        changes.append(_sync(conn, f"customer_{prefix}_product_sales", (
            ...
            WITH delta AS (
                ...
                FROM alibaba.customer_{prefix}_product_sales target
                JOIN upload_alibaba_periods periods
                  ON periods.grain = '{grain}'
                 AND periods.period_start = target.period_start
                 AND periods.period_end = target.period_end
    ...
    if len(changes) != 33:
        raise ValueError(f"阿里巴巴增量刷新表数异常：{len(changes)} != 33")
    return changes
