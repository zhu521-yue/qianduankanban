...
from upload.doudian_children.refresh import refresh_store
...
def commit_upload(
    ...
    """Execute the complete Children-store refresh inside the caller transaction."""
    if config.store_key != "doudian_children":
        raise ValueError("当前原子刷新器只支持抖店儿童服饰旗舰店")
