...
from upload.jushuitan.aggregate_refresh import refresh_aggregates
from upload.jushuitan.incremental_refresh import refresh_store_incremental
...
def commit_upload(
    ...
    if config.store_key != "jushuitan":
        raise ValueError("当前原子刷新器只支持聚水潭")
    ...
    if not affected_dates:
        raise ValueError("聚水潭上传没有可刷新的业务日期")
    ...
    if len(store_changes) != EXPECTED_STORE_TABLES:
        raise ValueError(
            f"聚水潭派生表刷新数量异常：{len(store_changes)} != {EXPECTED_STORE_TABLES}"
        )
