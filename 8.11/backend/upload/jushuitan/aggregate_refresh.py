...
def _scope_daily(expected: str) -> str:
    return f"SELECT expected.* FROM ({expected}) expected JOIN upload_jushuitan_dates dates USING (transaction_date)"
...
def refresh_fenxiao(
    ...
    changes = [_sync_scoped(
        ...
        "target.transaction_date IN (SELECT transaction_date FROM upload_jushuitan_dates)",
    )]
    ...
    for prefix, rate, interval in (
        ...
        changes.append(_sync_scoped(
            ...
            f"EXISTS (SELECT 1 FROM upload_jushuitan_periods p WHERE p.grain='{grain}' AND p.period_start=target.period_start AND p.period_end=target.period_end)",
        ))
    ...
    fenxiao_health_expected = '''
        SELECT 'natural_week'::text AS health_grain,
               'jushuitan'::text AS source_platform,
        ...
        FROM jushuitan.customer_health_detail health
    ...
    changes.append(_sync_scoped(
        ...
        "EXISTS (SELECT 1 FROM upload_jushuitan_periods p WHERE p.grain='half' AND p.period_start=target.period_start AND p.period_end=target.period_end)",
    ))
    return changes
