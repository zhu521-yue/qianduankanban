...
def commit_upload(
    ...
    if config.store_key != "youzan_qijian" or config.schema_name != "qijian":
        raise ValueError("当前原子刷新器只支持有赞旗舰店(qijian)")
    ...
    if len(store_changes) != EXPECTED_STORE_TABLES:
        raise ValueError(
            f"有赞旗舰店派生表刷新数量异常：{len(store_changes)} != {EXPECTED_STORE_TABLES}"
        )
