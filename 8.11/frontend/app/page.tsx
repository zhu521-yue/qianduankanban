...
const pagesByRole: Record<Role, PageConfig[]> = {
  talent: [
    ...
    { key: "settings", route: "#/talent/settings", kind: "settings", title: "达人组看板设置", navLabel: "设置", section: "系统", breadcrumb: ["达人组", "设置"], subtitle: "查看数据库健康规则" },
  ],
  private: [
    ...
    { key: "settings", route: "#/private/settings", kind: "settings", title: "私域组看板设置", navLabel: "设置", section: "系统", breadcrumb: ["私域组", "设置"], subtitle: "查看数据库健康规则" },
  ],
  distribution: [
    ...
    { key: "settings", route: "#/distribution/settings", kind: "settings", title: "分销组看板设置", navLabel: "设置", section: "系统", breadcrumb: ["分销组", "设置"], subtitle: "查看数据库健康规则" },
  ],
  manager: [
    ...
    { key: "settings", route: "#/manager/settings", kind: "settings", title: "主管端设置", navLabel: "主管端设置", section: "系统", breadcrumb: ["主管", "设置"], subtitle: "查看系统设置与客户状态规则" },
  ],
};
...
function Sidebar({ user, pages, activePage, onNavigate, onLogout }: { user: User; pages: PageConfig[]; activePage: PageConfig; onNavigate: (page: PageConfig) => void; onLogout: () => void }) {
  ...
  return (
    <aside className="sidebar hierarchical-sidebar">
      <div className="brand"><div className="brand-mark">看</div><div><strong>客户看板</strong><span>Customer Dashboard</span></div></div>
...
function CustomerTable({ scopeKey, asOf, onSelect }: { scopeKey: string; asOf: string; onSelect: (customer: CustomerListItem) => void }) {
  ...
  return (
    <section className="panel customer-table-panel">
      <SectionHeader eyebrow="CUSTOMER LIST" title={`${data ? `${dateText(data.period.start)}—${dateText(data.period.end)}` : "半年"}客户销售表现`} action={<div className="table-actions"><label><span>⌕</span><input value={query} onChange={event => { setQuery(event.target.value); setPage(1); }} placeholder="输入客户ID筛选" aria-label="输入客户ID筛选" /></label><span className="readonly-pill">数据库实时查询</span></div>} />
...
function DashboardPage({ page, onSelect }: { page: PageConfig; onSelect: (customer: CustomerListItem) => void }) {
  ...
  return (
    <main className="main-content">
      ...
      <footer className="page-footer">客户经营看板 · 数据由 weidian 数据库实时驱动</footer>
    </main>
  );
}
...
function CustomerDetailPage({ page, target, onBack }: { page: PageConfig; target: { storeKey: string; customerId: string }; onBack: () => void }) {
  ...
  return (
    <main className="main-content detail-content">
      ...
      <div className="detail-layout"><section className="detail-data"><CustomerDimensionPanel customer={customer} /></section></div>
    </main>
  );
}
...
function UploadPage({ page }: { page: PageConfig }) {
  ...
  return (
    <main className="main-content upload-page"><header className="topbar"><div><Breadcrumbs items={page.breadcrumb} /><h1>{page.title}</h1><p>{page.subtitle}</p></div><div className="upload-mode"><i></i>前端上传 · 预览后确认</div></header><div className="upload-notice"><strong>上传说明</strong><span>在当前网页选择 xlsx / csv 文件后先查看完整影响预览。只有已完成原子刷新开发的店铺会显示“确认写入数据库”；写入时任一表失败都会整体回滚。</span></div>{error && <div className="api-error-banner">{error}</div>}{!meta && !error && <LoadingPanel text="正在读取账号可用店铺…" />}{groups.map(([platform, stores]) => <section className="panel upload-platform-section" key={platform}><div className="upload-section-head"><div className="platform-emblem">{platform.slice(0, 1)}</div><div><span>{stores[0].platform_key.toUpperCase()}</span><h2>{platform}</h2></div><small>{stores.length} 个店铺入口</small></div><div className={`upload-card-grid ${stores.length > 1 ? "two" : ""}`}>{stores.map(store => <UploadCard key={store.key} store={store} maxBytes={meta!.upload.max_bytes} />)}</div></section>)}<footer className="page-footer">客户经营看板 · 文件从当前网页上传，预览与正式写入分两步执行</footer></main>
  );
}
...
function SettingsPage({ page, user }: { page: PageConfig; user: User }) {
  ...
  useEffect(() => {
    api.healthRules()
      .then(value => { setRuleGroups(normalizeHealthRuleGroups(value.groups)); setLoaded(true); })
  ...
  return (
    <main className="main-content settings-page">
      ...
      {!loaded && !error ? <LoadingPanel text="正在读取客户状态规则…" /> :
        <section className="settings-grid">          {ruleGroups.map(group =>
      ...
      <footer className="page-footer">客户经营看板 · 客户状态规则由业务组维护</footer>
    </main>
  );
}
...
function LoginPage({ ready, onLogin }: { ready: boolean; onLogin: (username: string, password: string) => Promise<void> }) {
  ...
  return (
    <main className="login-page"><section className="login-card" aria-labelledby="login-title"><div className="login-brand"><div className="brand-mark">看</div><div><strong>客户看板</strong><span>Customer Dashboard</span></div></div><div className="login-heading"><span>ACCOUNT ACCESS</span><h1 id="login-title">登录客户看板</h1><p>请输入分配给你的账号和密码，系统将从后端验证并进入对应组别。</p></div><form onSubmit={submit} noValidate><label><span>账号</span><input value={username} onChange={event => { setUsername(event.target.value); setError(""); }} autoComplete="username" placeholder="请输入账号" aria-invalid={Boolean(error)} /></label><label><span>密码</span><input type="password" value={password} onChange={event => { setPassword(event.target.value); setError(""); }} autoComplete="current-password" placeholder="请输入密码" aria-invalid={Boolean(error)} /></label><div className={`login-error ${error ? "visible" : ""}`} role="alert" aria-live="polite">{error || "\u00a0"}</div><button type="submit" disabled={!ready || submitting || !username.trim() || !password}>{!ready ? "正在连接后端…" : submitting ? "登录验证中…" : "登录并进入对应组别"}</button></form><footer><i></i><span>账号由后端独立配置文件管理</span></footer></section></main>
  );
}
